...
flagstatNano <- readLines("forjody/flagstat.txt")
  ...
  ##process sample names to remove extra chars
  ...
  ##process and format the reads and mapping info
  totalReads <- word(totalReads,1)
  mapped <- word(mapped,5)
  mapped <- gsub('\\(',"", mapped)
  
  ##create dataframes for the sequencers and set colnames
...
##merge the seqdata with another depth dataframe
...
##preprocess a list of sample anmes and add them to the df
...
##plot the boxplot for average depth
library(ggplot2)
library(reshape2)
...
#read combined data and split into separate data frames for Nanopore and Illumina
data <- read.csv("datacombined.csv",check.names = FALSE,header=TRUE,stringsAsFactors = FALSE)

split_dataframe <- function(df) {
  ##function to split the dataframe into Nanopore and Illumina specific dataframes
